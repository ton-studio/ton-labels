...
if __name__ == "__main__":
    ...
    for path in Path('assets').rglob('*.json'):
        with open(path, 'rt') as f:
            ...
            if type(data) == dict:
                data = [data]
            assert type(data) == list, f"Expected list, got {type(data)}"
            for item in data:
                try:
                    assert 'address' in item, f"Expected 'address' in {item} ({path})"
                    assert 'label' in item, f"Expected 'label' in {item} ({path})"
                    assert 'submittedBy' in item, f"Expected 'submittedBy' in {item} ({path})"
                    assert 'submissionTimestamp' in item, f"Expected 'submissionTimestamp' in {item} ({path})"

                    assert 'category' in item, f"Expected 'category' in {item} ({path})"
                    assert item['category'] in categories, f"Unknown category {item['category']} in {item} ({path})"
                    ...
                    output.append({
                        ...
                        'submissionTimestamp': item.get('submissionTimestamp', None),
                        'github_hash': sha
                    })
                except Exception as e:
                    logger.error(f"Error processing {path}: {e}: {traceback.format_exc()}")
                    raise e

    logger.info(f"Building output files with {len(output)} records")
    output = pd.DataFrame(output)
    os.makedirs('output', exist_ok=True)
    output.to_csv('output/assets.csv', index=False)
    with open('output/assets.json', 'wt') as f:
